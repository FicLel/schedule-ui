...
const DeleteUser = ({day, hourInterval, close, teacher, updateTable}) => {
  ...
  const handleTeacher = async () => {
    if (teacher?._id) {
      const tempTeacherBusyDays = teacher.busyDays.map((day) => ({
        day: day.day,
        start: transformmMinutesToHours(day.start),
        end: transformmMinutesToHours(day.end),
      }));
      tempTeacherBusyDays.push({day, start: hourInterval.start, end: hourInterval.end});
  ...
  return (
    ...
    >
      ...
      <Row>
        <Text>
          Estas a punto de seleccionar como horario no disponible 
      ...
      <Grid.Container justify="space-between" alignContent="center" css={{marginTop: '9px'}}>
        ...
        <Grid>
          <Button size="sm" shadow color="primary" onClick={() => handleTeacher()}>
            Asignar
          </Button>
        </Grid>
      </Grid.Container>
...
export const PopSetBusyDay = ({day, hours, teacher, updateTable}) => {
    ...
    <Grid.Container gap={2} alignContent="center" justify='center'>
      <Grid>
        <Popover isOpen={isOpen} onOpenChange={setIsOpen}>
          <Popover.Trigger>
            <Button light color="primary" auto>Ocupar</Button>
          </Popover.Trigger>
          <Popover.Content>
            <DeleteUser 
